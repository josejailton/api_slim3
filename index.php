...
$app->get('/{version}/product/{id}/{behaviour}', function (Request $request, Response $response, $parameters) use ($database) {

    $controller = new ProductController($request, $response, $database);

    return $controller->read($parameters['id'], $parameters['behaviour']);
});
